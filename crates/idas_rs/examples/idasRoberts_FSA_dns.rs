//! Port of `examples/idas/serial/idasRoberts_FSA_dns.c`.
//!
//! Example problem:
//!
...
fn main() {
    ...
    let mut yQS: Vec<N_Vector> = Vec::new();
    ...
    if sensi {
        yQS = N_VCloneVectorArray(NS, &yQ).expect("N_VCloneVectorArray");
        for is in 0..NS as usize {
    ...
    fname.push_str(".csv");
    let mut FID = SUNFile::fopen(&fname, "w");
    retval = IDAPrintAllStats(&ida_mem, &FID, SUNOutputFormat::SUN_OUTPUTFORMAT_CSV);
    let _ = retval;
