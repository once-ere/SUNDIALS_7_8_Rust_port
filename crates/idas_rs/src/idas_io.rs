//! Port of `src/idas/idas_io.c` — the optional input and output functions
//! for the IDAS solver (core, IC-calculation, quadrature, forward
//! sensitivity and quadrature-sensitivity getters/setters).
//!
//! Fragment protocol (see `idas_impl.rs`): the module-scope `#define`s that
//! `idas_io.c` repeats (`ZERO`, `HALF`, `ONE`, `TWOPT5`) are NOT redefined
//! here; they come from `crate::idas_impl::*`.
//!
...
//! Where the C code passes `NULL` (rather than `IDA_mem`) as the first
//! `IDAProcessError` argument — `IDASetRootDirection`, `IDASetQuadErrCon`,
//! `IDASetMaxNonlinIters`, `IDASetSensMaxNonlinIters` — the port passes
//! `None`, preserving the upstream (inconsistent) behaviour byte-for-byte.
//!
...
//!
//! Vector-array conventions (contract §4): C `N_Vector*` in-parameters that
//! are never NULL-tested become `&[N_Vector]`; the NULL-tested ones
//! (`IDAGetSensConsistentIC`) become `Option<&[N_Vector]>`; C `N_Vector**`
//! out-params (`IDAGetCurrentYSens`) become `&mut Vec<N_Vector>` holding
//! handle clones.
//!
...
//! such site copies the needed fields (or clones the needed handles) into
//! locals, drops the guard, then calls.
...
pub fn IDASetMaxNonlinIters(ida_mem: &IDAMem, maxcor: i32) -> i32 {
    ...
    let (sensi_sim, NLSsim, NLS) = {
        let IDA_mem = ida_mem.borrow();
        (
            IDA_mem.ida_sensi && (IDA_mem.ida_ism == IDA_SIMULTANEOUS),
            IDA_mem.NLSsim.clone(),
            IDA_mem.NLS.clone(),
        )
    };
    ...
    if sensi_sim {
        ...
        SUNNonlinSolSetMaxIters(&NLSsim, maxcor)
    } else {
        /* check that the NLS is non-NULL */
        let NLS = match NLS {
            Some(NLS) => NLS,
...
pub fn IDASetQuadErrCon(ida_mem: &IDAMem, errconQ: sunbooleantype) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadMallocDone == SUNFALSE {
        /* C passes NULL (not IDA_mem) here — preserved. */
        IDAProcessError(
            None,
            IDA_NO_QUAD,
    ...
    ida_mem.borrow_mut().ida_errconQ = errconQ;
...
pub fn IDASetSensDQMethod(ida_mem: &IDAMem, DQtype: i32, DQrhomax: sunrealtype) -> i32 {
    /* NULL-mem check: handled by type system */
...
pub fn IDASetSensErrCon(ida_mem: &IDAMem, errconS: sunbooleantype) -> i32 {
    /* NULL-mem check: handled by type system */
    ida_mem.borrow_mut().ida_errconS = errconS;
...
pub fn IDASetSensMaxNonlinIters(ida_mem: &IDAMem, maxcorS: i32) -> i32 {
    /* NULL-mem check: handled by type system */

    /* check that the NLS is non-NULL */
    let NLSstg = ida_mem.borrow().NLSstg.clone();

    let NLSstg = match NLSstg {
        Some(NLSstg) => NLSstg,
        None => {
            /* C passes NULL (not IDA_mem) here — preserved. */
            IDAProcessError(
    ...
    SUNNonlinSolSetMaxIters(&NLSstg, maxcorS)
}
...
pub fn IDASetSensParams(
    ...
    if ida_mem.borrow().ida_sensMallocDone == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
            IDA_NO_SENS,
            line!() as i32,
    ...
    match pbar {
        Some(pbar) => {
            for is in 0..Ns {
                ...
                ida_mem.borrow_mut().ida_pbar[is as usize] = SUNRabs(pbar[is as usize]);
            }
        }
        None => {
            let mut IDA_mem = ida_mem.borrow_mut();
            for is in 0..Ns {
                IDA_mem.ida_pbar[is as usize] = ONE;
            }
        }
    }
    ...
    match plist {
        Some(plist) => {
            for is in 0..Ns {
                ...
                ida_mem.borrow_mut().ida_plist[is as usize] = plist[is as usize];
            }
        }
        None => {
            let mut IDA_mem = ida_mem.borrow_mut();
            for is in 0..Ns {
                IDA_mem.ida_plist[is as usize] = is;
            }
        }
    }
 ...
 * -----------------------------------------------------------------
 */
pub fn IDASetQuadSensErrCon(ida_mem: &IDAMem, errconQS: sunbooleantype) -> i32 {
    /* NULL-mem check: handled by type system */
    ...
    if ida_mem.borrow().ida_quadSensMallocDone == SUNFALSE {
        /* upstream emits MSG_NO_SENSI (not MSG_NO_QUADSENSI) here */
        IDAProcessError(
            Some(ida_mem),
            IDA_NO_QUADSENS,
            line!() as i32,
    ...
    ida_mem.borrow_mut().ida_errconQS = errconQS;
...
pub fn IDAGetCurrentYpSens(ida_mem: &IDAMem, ypS: &mut Vec<N_Vector>) -> i32 {
    /* NULL-mem check: handled by type system */
    /* handle clones out = C array-pointer copy */
    *ypS = ida_mem.borrow().ida_ypS.clone();
...
pub fn IDAGetQuadNumRhsEvals(ida_mem: &IDAMem, nrQevals: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
            IDA_NO_QUAD,
            line!() as i32,
    ...
    *nrQevals = ida_mem.borrow().ida_nrQe;
...
pub fn IDAGetQuadNumErrTestFails(ida_mem: &IDAMem, nQetfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nQetfails = ida_mem.borrow().ida_netfQ;
...
pub fn IDAGetQuadErrWeights(ida_mem: &IDAMem, eQweight: &N_Vector) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    let ewtQ = {
        ...
        if IDA_mem.ida_errconQ {
            Some(IDA_mem.ida_ewtQ.as_ref().expect("ida_ewtQ").clone())
        } else {
            None
        }
    };
...
pub fn IDAGetQuadStats(ida_mem: &IDAMem, nrQevals: &mut i64, nQetfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
...
pub fn IDAGetQuadSensNumRhsEvals(ida_mem: &IDAMem, nrhsQSevals: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    ...
    *nrhsQSevals = ida_mem.borrow().ida_nrQSe;
...
pub fn IDAGetQuadSensNumErrTestFails(ida_mem: &IDAMem, nQSetfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nQSetfails = ida_mem.borrow().ida_netfQS;
...
pub fn IDAGetQuadSensErrWeights(ida_mem: &IDAMem, eQSweight: &[N_Vector]) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
...
pub fn IDAGetQuadSensStats(ida_mem: &IDAMem, nrhsQSevals: &mut i64, nQSetfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_quadr_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
...
pub fn IDAGetSensConsistentIC(
    ida_mem: &IDAMem,
    yyS0: Option<&[N_Vector]>,
    ypS0: Option<&[N_Vector]>,
) -> i32 {
    /* NULL-mem check: handled by type system */
    ...
    if let Some(yyS0) = yyS0 {
        let (Ns, phiS0) = {
            let IDA_mem = ida_mem.borrow();
            (IDA_mem.ida_Ns, IDA_mem.ida_phiS[0].clone())
        };
        for is in 0..Ns {
            N_VScale(ONE, &phiS0[is as usize], &yyS0[is as usize]);
        }
    }
    ...
    if let Some(ypS0) = ypS0 {
        let (Ns, phiS1) = {
            let IDA_mem = ida_mem.borrow();
            (IDA_mem.ida_Ns, IDA_mem.ida_phiS[1].clone())
        };
        for is in 0..Ns {
            N_VScale(ONE, &phiS1[is as usize], &ypS0[is as usize]);
        }
    }
...
pub fn IDAGetSensNumResEvals(ida_mem: &IDAMem, nrSevals: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nrSevals = ida_mem.borrow().ida_nrSe;
...
pub fn IDAGetNumResEvalsSens(ida_mem: &IDAMem, nrevalsS: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nrevalsS = ida_mem.borrow().ida_nreS;
...
pub fn IDAGetSensNumErrTestFails(ida_mem: &IDAMem, nSetfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nSetfails = ida_mem.borrow().ida_netfS;
...
pub fn IDAGetSensNumLinSolvSetups(ida_mem: &IDAMem, nlinsetupsS: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nlinsetupsS = ida_mem.borrow().ida_nsetupsS;
...
pub fn IDAGetSensErrWeights(ida_mem: &IDAMem, eSweight: &[N_Vector]) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    let (Ns, ewtS) = {
        let IDA_mem = ida_mem.borrow();
        (IDA_mem.ida_Ns, IDA_mem.ida_ewtS.clone())
    };
...
pub fn IDAGetSensNumNonlinSolvIters(ida_mem: &IDAMem, nSniters: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nSniters = ida_mem.borrow().ida_nniS;
...
pub fn IDAGetSensNumNonlinSolvConvFails(ida_mem: &IDAMem, nSnfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
    ...
    *nSnfails = ida_mem.borrow().ida_nnfS;
...
pub fn IDAGetSensNonlinSolvStats(ida_mem: &IDAMem, nSniters: &mut i64, nSnfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    if ida_mem.borrow().ida_sensi == SUNFALSE {
        IDAProcessError(
            Some(ida_mem),
...
pub fn IDAGetNumStepSensSolveFails(ida_mem: &IDAMem, nSncfails: &mut i64) -> i32 {
    /* NULL-mem check: handled by type system */
    /* upstream reads ida_ncfn (NOT ida_ncfnS) here — preserved */
    *nSncfails = ida_mem.borrow().ida_ncfn;
...
pub fn IDAPrintAllStats(ida_mem: &IDAMem, outfile: &SUNFile, fmt: SUNOutputFormat) -> i32 {
    ...
    let sensi_stg_stats: Option<(i64, i64, i64)>;
    let quadr_sensi_stats: Option<(i64, i64)>;
    {
        ...
        quadr_stats = if IDA_mem.ida_quadr {
            Some((IDA_mem.ida_nrQe, IDA_mem.ida_netfQ))
        } else {
            None
        };
        ...
        if IDA_mem.ida_sensi {
            ...
            sensi_stg_stats = if IDA_mem.ida_ism == IDA_STAGGERED {
                Some((IDA_mem.ida_nniS, IDA_mem.ida_nnfS, IDA_mem.ida_ncfnS))
            } else {
                None
            };
        } else {
            sensi_stats = None;
            sensi_stg_stats = None;
        }

        quadr_sensi_stats = if IDA_mem.ida_quadr_sensi {
            Some((IDA_mem.ida_nrQSe, IDA_mem.ida_netfQS))
        } else {
            None
        };
    }
    ...
    /* quadrature stats */
    if let Some((nrQe, netfQ)) = quadr_stats {
        sunfprintf_long(outfile, fmt, SUNFALSE, "Quad fn evals", nrQe);
        sunfprintf_long(outfile, fmt, SUNFALSE, "Quad error test fails", netfQ);
    }

    /* sensitivity stats */
    if let Some((nrSe, nreS, netfS, nsetupsS)) = sensi_stats {
        sunfprintf_long(outfile, fmt, SUNFALSE, "Sens fn evals", nrSe);
        sunfprintf_long(outfile, fmt, SUNFALSE, "Sens residual fn evals", nreS);
        sunfprintf_long(outfile, fmt, SUNFALSE, "Sens error test fails", netfS);
        if let Some((nniS, nnfS, ncfnS)) = sensi_stg_stats {
    ...
    if let Some((nrQSe, netfQS)) = quadr_sensi_stats {
        sunfprintf_long(outfile, fmt, SUNFALSE, "QuadSens residual evals", nrQSe);
...
pub fn IDAGetReturnFlagName(flag: i64) -> String {
    let name = match flag {
        ...
        f if f == IDA_NO_SENS as i64 => "IDA_NO_SENS",
        f if f == IDA_SRES_FAIL as i64 => "IDA_SRES_FAIL",
        f if f == IDA_REP_SRES_ERR as i64 => "IDA_REP_SRES_ERR",
        f if f == IDA_BAD_IS as i64 => "IDA_BAD_IS",
        f if f == IDA_NO_QUAD as i64 => "IDA_NO_QUAD",
        f if f == IDA_NO_QUADSENS as i64 => "IDA_NO_QUADSENS",
        f if f == IDA_QRHS_FAIL as i64 => "IDA_QRHS_FAIL",
        f if f == IDA_REP_QRHS_ERR as i64 => "IDA_REP_QRHS_ERR",
        f if f == IDA_QSRHS_FAIL as i64 => "IDA_QSRHS_FAIL",
        f if f == IDA_REP_QSRHS_ERR as i64 => "IDA_REP_QSRHS_ERR",

        /* IDAA flags follow below. */
        f if f == IDA_NO_ADJ as i64 => "IDA_NO_ADJ",
        f if f == IDA_BAD_TB0 as i64 => "IDA_BAD_TB0",
        f if f == IDA_REIFWD_FAIL as i64 => "IDA_REIFWD_FAIL",
        f if f == IDA_FWD_FAIL as i64 => "IDA_FWD_FAIL",
        f if f == IDA_GETY_BADT as i64 => "IDA_GETY_BADT",
        f if f == IDA_NO_BCK as i64 => "IDA_NO_BCK",
        f if f == IDA_NO_FWD as i64 => "IDA_NO_FWD",
