/* --------------------------------------------------------------------
 * Rust port of examples/arkode/C_serial/ark_KrylovDemo_prec.c
 * --------------------------------------------------------------------
 ...
 * unit square. The preconditioner is a product of (1) Gauss-Seidel
...
fn main() {
    ...
    if argc > 1 {
        nrmfactor = argv[1].trim().parse().unwrap_or(0);
    }

    /* Initializations */
    let c = N_VNew_Serial(NEQ as sunindextype, &ctx);
    if check_flag_ptr(&c, "N_VNew_Serial", 0) != 0 {
        std::process::exit(1);
    }
    let c = c.expect("N_VNew_Serial");
    let wdata = AllocUserData(&ctx);
    if check_flag_ptr(&wdata, "AllocUserData", 2) != 0 {
        std::process::exit(1);
    }
    ...
    /* user_data box handed to ARKODE on the first run; retrieved by swap
     * (ARKodeGetUserData) whenever main needs it again. */
    ...
    for jpre in SUN_PREC_LEFT..=SUN_PREC_RIGHT {
        for gstype in SUN_MODIFIED_GS..=SUN_CLASSICAL_GS {
            ...
            if firstrun {
                ...
                let mem = arkode_mem.as_ref().expect("arkode_mem");
                ARKodeGetUserData(mem, &mut wdata_any);
                {
                    let wdata_ref = wdata_any
                        .as_ref()
                        .and_then(|b| b.downcast_ref::<WebData>())
                ...
                ARKodeGetUserData(mem, &mut wdata_any); /* hand the box back */
            }
            ...
            if firstrun {
                arkode_mem = ARKStepCreate(None, Some(f), T0, &c, &ctx);
                ...
                wdata_any
                    ...
                    .expect("user_data is WebData")
                    .arkode_mem = Some(mem.clone());

                let flag = ARKodeSetUserData(mem, wdata_any.take());
                if check_flag(flag, "ARKodeSetUserData", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = ARKodeSStolerances(mem, reltol, abstol);
                if check_flag(flag, "ARKodeSStolerances", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = ARKodeSetMaxNumSteps(mem, 1000);
                if check_flag(flag, "ARKodeSetMaxNumSteps", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = ARKodeSetNonlinConvCoef(mem, 1.0e-3);
                if check_flag(flag, "ARKodeSetNonlinConvCoef", 1) != 0 {
                    std::process::exit(1);
                }
                ...
                let flag = ARKodeSetLinearSolver(mem, ls, None);
                if check_flag(flag, "ARKodeSetLinearSolver", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = SUNLinSol_SPGMRSetGSType(ls, gstype);
                if check_flag(flag, "SUNLinSol_SPGMRSetGSType", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = ARKodeSetEpsLin(mem, DELTA);
                if check_flag(flag, "ARKodeSetEpsLin", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = ARKodeSetPreconditioner(mem, Some(Precond), Some(PSolve));
                if check_flag(flag, "ARKodeSetPreconditioner", 1) != 0 {
                    std::process::exit(1);
                }
                ...
                match nrmfactor {
                    1 => {
                        ...
                        nrmfac = SUNRsqrt(NEQ as sunrealtype);
                    }
                    2 => {
                ...
                let flag = ARKodeSetLSNormFactor(mem, nrmfac);
                if check_flag(flag, "ARKodeSetLSNormFactor", 1) != 0 {
                    std::process::exit(1);
                }
            } else {
                let mem = arkode_mem.as_ref().expect("arkode_mem");
                ...
                let flag = ARKStepReInit(mem, None, Some(f), T0, &c);
                if check_flag(flag, "ARKStepReInit", 1) != 0 {
                    std::process::exit(1);
                }

                let flag = SUNLinSol_SPGMRSetPrecType(ls, jpre);
                if check_flag(flag, "SUNLinSol_SPGMRSetPrecType", 1) != 0 {
                    std::process::exit(1);
                }
            ...
            tout = T1;
            for iout in 1..=NOUT {
                let flag = ARKodeEvolve(mem, tout, &c, &mut t, ARK_NORMAL);
                PrintOutput(mem, t);
...
fn AllocUserData(ctx: &SUNContext) -> Option<WebData> {
    let ns = NS as usize;
    ...
    let rewt = N_VNew_Serial(NEQ as sunindextype, ctx)?;
    let tmp = N_VNew_Serial(NEQ as sunindextype, ctx)?;
...
fn PrintOutput(arkode_mem: &ARKodeMem, t: sunrealtype) {
    let mut nst: i64 = 0;
    let mut nfe: i64 = 0;
    let mut nfi: i64 = 0;
    let mut nni: i64 = 0;
    ...
    let flag = ARKodeGetNumSteps(arkode_mem, &mut nst);
    check_flag(flag, "ARKodeGetNumSteps", 1);
    let flag = ARKodeGetNumRhsEvals(arkode_mem, 0, &mut nfe);
    check_flag(flag, "ARKodeGetNumRhsEvals", 1);
    let flag = ARKodeGetNumRhsEvals(arkode_mem, 1, &mut nfi);
    check_flag(flag, "ARKodeGetNumRhsEvals", 1);
    let flag = ARKodeGetNumNonlinSolvIters(arkode_mem, &mut nni);
    check_flag(flag, "ARKodeGetNumNonlinSolvIters", 1);
    let flag = ARKodeGetLastStep(arkode_mem, &mut hu);
...
fn PrintFinalStats(arkode_mem: &ARKodeMem) {
    let mut lenrw: i64 = 0;
    let mut leniw: i64 = 0;
    ...
    let flag = ARKodeGetWorkSpace(arkode_mem, &mut lenrw, &mut leniw);
    ...
    let flag = ARKodeGetNumRhsEvals(arkode_mem, 1, &mut nfi);
    check_flag(flag, "ARKodeGetNumRhsEvals", 1);
    let flag = ARKodeGetNumLinSolvSetups(arkode_mem, &mut nsetups);
    check_flag(flag, "ARKodeGetNumLinSolvSetups", 1);
    let flag = ARKodeGetNumErrTestFails(arkode_mem, &mut netf);
    check_flag(flag, "ARKodeGetNumErrTestFails", 1);
    let flag = ARKodeGetNumNonlinSolvIters(arkode_mem, &mut nni);
    check_flag(flag, "ARKodeGetNumNonlinSolvIters", 1);
    let flag = ARKodeGetNumNonlinSolvConvFails(arkode_mem, &mut ncfn);
    check_flag(flag, "ARKodeGetNumNonlinSolvConvFails", 1);

    let flag = ARKodeGetLinWorkSpace(arkode_mem, &mut lenrwLS, &mut leniwLS);
    check_flag(flag, "ARKodeGetLinWorkSpace", 1);
    let flag = ARKodeGetNumLinIters(arkode_mem, &mut nli);
    check_flag(flag, "ARKodeGetNumLinIters", 1);
    let flag = ARKodeGetNumPrecEvals(arkode_mem, &mut npe);
    check_flag(flag, "ARKodeGetNumPrecEvals", 1);
    let flag = ARKodeGetNumPrecSolves(arkode_mem, &mut nps);
    check_flag(flag, "ARKodeGetNumPrecSolves", 1);
    let flag = ARKodeGetNumLinConvFails(arkode_mem, &mut ncfl);
    check_flag(flag, "ARKodeGetNumLinConvFails", 1);
    let flag = ARKodeGetNumLinRhsEvals(arkode_mem, &mut nfeLS);
    check_flag(flag, "ARKodeGetNumLinRhsEvals", 1);

    print!("\n\n Final statistics for this run:\n\n");
    print!(" ARKStep real workspace length         = {:4} \n", lenrw);
    print!(" ARKStep integer workspace length      = {:4} \n", leniw);
    print!(" ARKLS real workspace length           = {:4} \n", lenrwLS);
    print!(" ARKLS integer workspace length        = {:4} \n", leniwLS);
    print!(" Number of steps                       = {:4} \n", nst);
    print!(" Number of f-s (explicit)              = {:4} \n", nfe);
    print!(" Number of f-s (implicit)              = {:4} \n", nfi);
...
fn Precond(
    ...
    let arkode_mem = wdata.arkode_mem.as_ref().expect("arkode_mem").clone();
    let rewt = wdata.rewt.clone();
    let flag = ARKodeGetErrWeights(&arkode_mem, &rewt);
    if check_flag(flag, "ARKodeGetErrWeights", 1) != 0 {
        return 1;
    }
